module.exports.scoreHand = function(hand, isCrib) {
    ...
    var score = count15s(cards, result);
...
function count15s(fullHand, result)
{
    ...
    for (var i = 2; i < 6; i++)
    {
        ...
        for (var setLength = 0; setLength < _setMatrix[i].length; setLength++)
        {
            var sum = 0;

            var cards = new Array();
            // Add up the cards pointed to by the indicies of the set.
            for (var setMember = 0; setMember < sets[setLength].length; setMember++) {
...
function countRuns(fullHand, result)
{
    ...
    for (var i = 5; i > 2; i--)
    {
        ...
        for (var setLength = 0; setLength < sets.length; setLength++)
        {
            var localRunFound = true;

            // Look at each each index in the set, from the first to the second-to-last.
            for (var setMember = 0; setMember < sets[setLength].length - 1; setMember++)
            {
                var first = fullHand[sets[setLength][setMember]];
                var second = fullHand[sets[setLength][setMember + 1]];
